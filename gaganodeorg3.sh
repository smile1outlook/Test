...
create_dock () {
  docker run -itd --name vpn$i --cap-add=NET_ADMIN --env BLOCK_MALICIOUS=off --env BLOCK_SURVEILLANCE=off --env BLOCK_ADS=off --env DOT=off --env VPN_SERVICE_PROVIDER=ipvanish --env OPENVPN_USER=c.marie.girard@gmail.com --env OPENVPN_PASSWORD=Girardo32 --env SERVER_COUNTRIES=${array[$index]} -p 90$i:90$i -p 514$i:514$i -p 514$i:514$i/udp qmcgaw/gluetun 
  sleep 10 
  sudo docker run -itd --name gaga$i --network=container:vpn$i --env TOKEN=ljayejvpzgflfxyg5fbbd7e29b191566 jepbura/gaganode
}
#i=$(($i+1))
