...
for j in {1..6}; do
  ...
  while docker logs --tail 4 gaga$i | grep -E 'vpn|err:|node config will|ERRO|command not found'; do docker restart vpn$i gaga$i && echo retrying.. $i $$ docker logs --tail 4 gaga$i && sleep 10; done && docker logs --tail 4 gaga$i
