...
array=("USA Central" "Indonesia" "South Korea" "USA East" "USA West") 
size=${#array[@]}
docker pull qmcgaw/gluetun
docker pull jepbura/gaganode
#-p 90$i:90$i -p 514$i:514$i -p 514$i:514$i/udp
create_dock () {
  docker run -itd --rm --cap-add=NET_ADMIN -e BLOCK_MALICIOUS=off -e BLOCK_SURVEILLANCE=off -e BLOCK_ADS=off -e DOT=off -e VPN_SERVICE_PROVIDER=windscribe -e VPN_TYPE=openvpn -e OPENVPN_USER=6sujvyhr-jpwncz4 -e OPENVPN_PASSWORD=h3dfk2wsy6 -e SERVER_REGIONS=${array[$index]} --name $vpnname qmcgaw/gluetun
