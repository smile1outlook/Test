...
for j in {1..6}; do
  ...
  until docker logs --tail 2 gaga$i | grep 'node started'; do if docker logs --tail 2 gaga$i | grep -E 'vpn|err:|node confirm will'; then docker restart vpn$i gaga$i && sleep 10; else echo retrying.. $i && sleep 0.5; fi; done
