...
for j in {1..6}; do
  ...
  until docker logs --tail 2 gaga$i | grep 'node started'; do if docker logs --tail 4 gaga$i | grep -E 'vpn|err:|node confirm will'|ERRO|command not found; then docker restart vpn$i gaga$i && sleep 10; else echo retrying.. $i && docker logs --tail 2 gaga$i && sleep 0.5; fi; done
