...
array=("US West" "US East" "US Central" "South Korea") && size=${#array[@]} && for i in {1..100}; do
  index=$(($RANDOM % $size))
  docker run -itd --rm --cap-add=NET_ADMIN --name vpn$i -e BLOCK_MALICIOUS=off -e BLOCK_SURVEILLANCE=off -e BLOCK_ADS=off -e DOT=off -e VPN_SERVICE_PROVIDER=windscribe -e VPN_TYPE=openvpn -e OPENVPN_USER="2jaentdo-gy479fv" -e OPENVPN_PASSWORD="u65s2k8d9b" -e SERVER_REGIONS="${array[$index]}" qmcgaw/gluetun
