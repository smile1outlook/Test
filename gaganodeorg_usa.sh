...
create_dock () {
  docker run -itd --name vpn$i --cap-add=NET_ADMIN --env BLOCK_MALICIOUS=off --env BLOCK_SURVEILLANCE=off --env BLOCK_ADS=off --env DOT=off --env VPN_SERVICE_PROVIDER=ipvanish --env OPENVPN_USER=toombz@yahoo.co.uk --env OPENVPN_PASSWORD=dragonball12 --env SERVER_COUNTRIES="United States" -p 8$i:8$i -p 52$i:52$i -p 52$i:52$i/udp qmcgaw/gluetun 
  sleep 10 
