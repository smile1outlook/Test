...
create_dock () {
  docker run -itd --name vpn$i --cap-add=NET_ADMIN -e BLOCK_MALICIOUS=off -e BLOCK_SURVEILLANCE=off -e BLOCK_ADS=off -e DOT=off -e VPN_SERVICE_PROVIDER=windscribe -e OPENVPN_USER="mgqt21sr-dmvtns9" -e OPENVPN_PASSWORD="uhma4qbxyn" -e SERVER_REGIONS="${array[$index]}" qmcgaw/gluetun 
  sleep 15 
