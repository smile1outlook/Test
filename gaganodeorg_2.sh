...
array=("US West" "US East" "US Central" "South Korea") 

size=${#array[@]}
docker pull qmcgaw/gluetun
docker pull jepbura/gaganode
create_dock () {
  docker run -itd --name vpn$i --cap-add=NET_ADMIN -e BLOCK_MALICIOUS=off -e BLOCK_SURVEILLANCE=off -e BLOCK_ADS=off -e DOT=off -e VPN_SERVICE_PROVIDER=windscribe -e OPENVPN_USER="mgqt21sr-3nkmzqf" -e OPENVPN_PASSWORD="3ymzg7uvs8" -e SERVER_REGIONS="${array[$index]}" qmcgaw/gluetun 
  sleep 15 
